...
def _make_page(page_id: str = "Atomic Layer Deposition", kind: str = "article") -> WikiPage:
    return WikiPage(
        ...
        aliases=[],
        body_markdown="",
        evidence=[Evidence(marker="e1", chunk_id="c1", doc_id="d1", quote="quote", locator="")],
        links=[],
        provenance={},
    )
...
def _make_cfg(
    style: str = "STYLE", field: str = "FIELD", artifact: str = "ARTIFACT"
) -> WriteRequestConfig:
...
def _make_images_index(tmp_path: Path) -> ImageIndex:
    return ImageIndex(corpus_root=tmp_path)
...
def test_compose_writer_prompt_layer_hashes_returns_three_keys() -> None:
    hashes = compose_writer_prompt_layer_hashes("materials_science", "wiki_article")
...
def test_compose_writer_prompt_layer_hashes_stable() -> None:
    ...
    h1 = compose_writer_prompt_layer_hashes("materials_science", "wiki_article")
    h2 = compose_writer_prompt_layer_hashes("materials_science", "wiki_article")
    assert h1 == h2


def test_compose_writer_prompt_layer_hashes_differ_by_field() -> None:
    h_ms = compose_writer_prompt_layer_hashes("materials_science", "wiki_article")
    h_gen = compose_writer_prompt_layer_hashes("generic", "wiki_article")
...
def test_pipeline_writes_prompt_layer_files(tmp_path: Path) -> None:
    ...
    from wikify.distill.pipeline import _write_prompt_layer_files
