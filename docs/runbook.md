...
A parallel Claude Code session running `/wikify/runtime/serve-dispatch` handles the `data/dispatch/` round-trips. See `test-run-playbook.md` for the full procedure.
...
- `/wikify/extract` -- reads `data/dispatch/extract/*.request.json`,
...
```bash
rm -rf data/corpora/mvp20_real
uv run python -m wikify.cli ingest \
  --source data/papers/mvp20 \
  --corpus data/corpora/mvp20_real
```
...
uv run python -m wikify.cli distill \
  --corpus data/corpora/mvp20_real \
  --bundle data/wikis/mvp20_real_M \
  --strategy M \
  --budget 300000 \
  --seed 0
```
...
```bash
watch -n 5 'cat data/wikis/mvp20_real_M/M_*/_run.json | python -m json.tool | head -40'
```
...
```bash
uv run python -m wikify.cli eval \
  --bundle data/wikis/mvp20_real_M \
  --corpus data/corpora/mvp20_real
```
...
```bash
uv run python -m wikify.cli query \
  --bundle data/wikis/mvp20_real_M \
  "what is a memristor?"
```
