...
1. **Fluent API.** Agents chain traversals like natural reasoning:
   `kg.source("X").cited_by().sections(type="conclusions").search("topic")`
...
class KnowledgeGraph:
    ...
    def sources(self, **filters) -> QueryBuilder     # all papers, optionally filtered
...
class QueryBuilder:
    ...
    # Citation graph
    def cited_by(self) -> QueryBuilder          # sources that cite these
    def references(self, ords=None) -> QueryBuilder  # sources cited by these
    def neighborhood(self, hops=1) -> QueryBuilder   # N-hop graph neighbors
    
    # Authorship
    def authors(self) -> QueryBuilder           # authors of these sources
    def sources(self) -> QueryBuilder            # sources by these authors
    def coauthors(self) -> QueryBuilder         # co-authors of these authors
    
    # Document structure
    def sections(self, type=None) -> QueryBuilder  # sections of these sources
    def chunks(self) -> QueryBuilder               # chunks of these sources/sections
    def figures(self) -> QueryBuilder               # figures of these sources
    def equations(self) -> QueryBuilder             # equations of these sources
...
kg.source("X").cited_by().top(20, by="year").collect()
...
kg.author("smith_j").sources().collect()

# UC6: "Authors who write papers similar to author A"
kg.author("smith_j").sources().search("", top_k=20).authors().collect()

# UC7: "Who does author A cite most?"
kg.author("smith_j").sources().references().authors().collect()
...
# UC12: "Author impact"
kg.author("smith_j").sources().cited_by().count()  # total citations
```
...
kg.sources().figures().search("I-V curve characteristic", top_k=5)
...
eq = kg.source("X").equations().where(label="Eq. 1").first()
context = kg.source("X").chunks().where(contains_equation=eq["id"]).collect()
...
chunk = kg.source("X").chunks().where(id="c_123").first()
...
kg.project("Icarus").sources().collect()

# "Emails about a topic that reference a specific paper"
kg.source("X").cited_by(type="email").collect()
...
SourceNode
  ...
  kind: "corpus" | "cited"
  citation_count: int
  ord_refs: dict[int, str]      # [N] -> target source_id
...
AuthorNode
  ...
  source_count, citation_count, h_index, pagerank: float
```
...
```
ChunkNode
  id, source_id, ord, section, section_type
...
SectionNode
  id: "{source_id}::{heading}"
  source_id, heading, level
...
```
FigureNode
  id, source_id, label, caption, path, page
  near_chunk_ids: list[str]

EquationNode
  id, source_id, latex, label, kind, context, chunk_id
```
...
COUPLES (bibliographic coupling), SIMILAR (embedding)
...
class QueryBuilder:
    ...
    def search(self, query: str, top_k: int = 10) -> list[dict]:
        ...
        return self._kg._vector_store.query(
            query_texts=[query],
            where={"source_id": {"$in": list(self._ids)}},
            n_results=top_k,
        )
...
| Full markdown text | Filesystem | `source.markdown_path` |
| Images (binary) | Filesystem | `figure.path` |
| DOI cache | data/doi_cache.db | Ingestion-time only |
| BibTeX files | Generated | `source.bibtex_key` |
...
def test_cited_by():
    ...
    result = kg.source("A").cited_by().collect()
    assert len(result) == 2
    assert {"B", "C"} == {r["id"] for r in result}

def test_search_scoped_to_citing_sources():
    kg = build_test_graph()
    results = kg.source("A").cited_by().chunks().search("topic", top_k=3)
    # All results come from papers that cite A
    assert all(r["source_id"] in kg.source("A").cited_by().ids() for r in results)

def test_fluent_chain():
    # Each step narrows the scope
    all_sources = kg.sources().count()
    cited = kg.source("X").cited_by().count()
    assert cited < all_sources
```
