...
| `src/wikify/baselines/` | `BaselineConfig`, evidence helper | C — `redesign/c-baselines-distill-debug-retire` | Strategy → skill frontmatter; helper moved in W3. |
| `src/wikify/distill/{__init__,preload,write_runner}.py` | aggregator + pre-load + post-commit graph rebuild | C — same PR | Logic moved into `draft/builder.py`, `wiki/commit.py`. |
...
| W10 | `redesign/c-*` (5 PRs in §7) | deletes `cli/legacy/*`, `session.py`/`meter.py` shells, `paths.py` shell, `baselines/`, residual `distill/{__init__,preload,write_runner}.py`, `citations/__main__.py`, legacy tests | legacy tests deleted | ruff+pytest clean; legacy nouns deregistered |
...
4. `redesign/c-baselines-distill-debug-retire` — delete `src/wikify/baselines/`, `src/wikify/distill/{__init__,preload,write_runner}.py`, `src/wikify/citations/__main__.py`. Helpers absorbed by `corpus/queries.py`, `draft/builder.py`, `wiki/commit.py`.
