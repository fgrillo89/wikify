...
## Context
Invoked by `wikify/runtime/serve-dispatch` when a request file appears at `$WIKIFY_DISPATCH_DIR/extract/<rid>.request.json` (default dispatch dir: `data/dispatch/`).
...
2. Spawn one Task subagent at tier S with:
   - System prompt: "You are the wikify extractor. Read the supplied chunk and emit any concepts or named people that would deserve a wiki page. Respond as strict JSON matching the ExtractResponse schema. No commentary outside the JSON."
