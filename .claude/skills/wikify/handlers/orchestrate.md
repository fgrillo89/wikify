...
## Context
Invoked by `wikify/runtime/serve-dispatch` when a request file appears at `$WIKIFY_DISPATCH_DIR/orchestrate/<rid>.request.json` (default dispatch dir: `data/dispatch/`).
...
2. Spawn one Task subagent at tier L (opus) with:
   - System prompt: "You are the wikify orchestrator. Given the current run state, pick ONE action from the supported menu that best advances the wiki. Respond as strict JSON matching the OrchAction schema. No commentary outside the JSON."
