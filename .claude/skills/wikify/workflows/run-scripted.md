---
name: wikify/workflows/run-scripted
...
## Steps
1. Verify `corpus` exists (`ls <corpus>/docs` should be non-empty). If missing, tell the user to run `wikify ingest` first.
...
6. If `render_html`, run `uv run python -m wikify.cli html --bundle $BUNDLE`.
7. If `run_eval`, run `uv run python -m wikify.cli eval --bundle $BUNDLE --corpus {corpus}`.
...
- The user must start `/wikify/runtime/serve-dispatch` in a second Claude session to handle dispatch round-trips.
