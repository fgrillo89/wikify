...
Simplified conventional-RAG build path. Owns sample count, concept
extraction, evidence quotas, writer tier, concurrency, retry policy,
render/eval closure, and stop conditions. Core capability skills
explain mechanics.
...
`../reference/references/exploration/concept-extraction.md`.
...
`gather-evidence` Task as a top-up. The new
...
# Invoke organize-wiki to write and apply derived/navigation.json.
...
| supervisor | sonnet | `gather-evidence` | `cluster_slugs`, `run`, `corpus`, `quota_per_slug=12`, `max_query_rounds=3` | per-slug envelope dict (≤600 tok) |
| chunk-judge | haiku | `gather-evidence` (judge role) | sibling slugs + batch of ≤8 chunks with text | per-chunk routing+score+quote JSON |
| writer | sonnet M | `write-page` | cluster slugs + dossier paths | per-slug `response.json` paths |
...
- Budget: ~22M haiku-eq on a 12–16-doc build (dominated by ~28
...
- Using graph-RAG, summaries, rerankers, or query refinement
  (off-protocol for this simple-RAG path).
...
- `../reference/references/exploration/concept-extraction.md`
- `../reference/references/exploration/sampling-patterns.md`
