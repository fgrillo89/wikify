...
description: Ask one question of a wiki bundle. Fast with heuristic binding, model-backed with file_dispatch.
---
...
| `binding` | `heuristic` | `fake` (stub), `heuristic` (fast, local), or `file_dispatch` (model-backed). |

## Steps

### Fast path (binding = heuristic)
1. Verify `bundle` exists.
2. Run: `uv run python -m wikify_simple.cli query --bundle {bundle} --binding heuristic "{question}"`
3. Print the result location (under `data/queries/<bundle_name>/<timestamp>.md`).

No Claude session cooperation needed — heuristic executes in-process.

### Quality path (binding = file_dispatch)
1. Verify `bundle` exists and `WIKIFY_SIMPLE_ALLOW_NETWORK=1` is set.
2. Verify a serve-dispatch session is running (or start one in parallel).
3. Run: `uv run python -m wikify_simple.cli query --bundle {bundle} --binding file_dispatch "{question}"`
...
- serve-dispatch not running (quality path) → harness hangs for 600s then times out.
