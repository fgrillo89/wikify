...
from wikify.papers.generate.artifact_types.registry import ARTIFACT_TYPES, get_artifact_type
from wikify.papers.generate.field_guide import detect_field, load_field_guide
...
def _no_db(monkeypatch):
    ...
    monkeypatch.setattr(
        "wikify.papers.generate.persona._get_top_topics",
        lambda limit=5: [],
    )
...
def test_generation_prompt_contains_deep_read():
    from wikify.papers.agent.defaults import build_generation_prompt
...
def test_explorer_prompt_prefers_hierarchical_reads():
    from wikify.papers.agent.defaults import build_explorer_prompt
