# wikify — Structural improvements from the test-run findings
...
- `src/wikify/paths.py` — add `sampler_index_path` and `pagerank_path` properties on `CorpusPaths`
- `src/wikify/ingest/refresh.py` — after building the corpus graph (existing line ~114), compute the pure sampler-state fields + real pagerank, serialize both
- `src/wikify/ingest/sampler_index.py` **NEW** — `build_sampler_index(docs, chunks, graph, vectors) -> dict` + `save_sampler_index(path, index)` + `load_sampler_index(path) -> dict | None`. The build function is extracted verbatim from `_build_sampler_state` so there's a single source of truth.
- `src/wikify/distill/pipeline.py::_build_sampler_state` — call `load_sampler_index(corpus.sampler_index_path)`; if present, assemble `SamplerState` directly from it; if missing (older corpus), fall back to the existing in-memory build and warn on stderr.
- `src/wikify/distill/pipeline.py::run` — for `phase == "write"`, skip the `read_vector_store`, `read_graph`, and sampler-state build entirely. The write phase only needs docs, chunks, images_index, and the write request manifest.
- `tests/wikify/test_sampler_index.py` **NEW** — round-trip build/save/load returns the same `SamplerState` fields as today's `_build_sampler_state`.
...
- `uv run python -m wikify.cli ingest data/papers/mvp20 --out data/corpora/mvp20_next` produces `sampler_index.json` + `pagerank.json`.
- `diff` the resulting `SamplerState` fields before/after the refactor on a fixture corpus — zero difference.
- `uv run pytest tests/wikify/test_sampler_index.py` passes.
- `uv run pytest tests/wikify/test_pipeline_reallocate.py` still passes (exercises pipeline.run on a tiny corpus).
...
- `src/wikify/cli.py` — new `campaign` command that wraps the existing `distill` body in a loop. The distill body is refactored into a helper `_run_one_iteration(...)` that takes pre-loaded state.
- `src/wikify/distill/pipeline.py` — extract a new `PreloadedCorpus` dataclass (`{docs, chunks_by_id, images_index, sampler_index, vectors, graph, ...}`) and a new top-level `run_with_preloaded(preloaded, bundle, strategy, ...)` that `pipeline.run` delegates to after loading. The CLI campaign verb calls `preload_corpus(corpus)` once and `run_with_preloaded` N times.
- `src/wikify/distill/preload.py` **NEW** — `preload_corpus(corpus: CorpusPaths) -> PreloadedCorpus`. Uses the sampler index from Phase 1 if present.
- `.claude/skills/wikify/workflows/run-scripted.md` — point at `wikify campaign` instead of looping distill calls.
- `.claude/skills/wikify/workflows/run-campaign.md` — same.
- `tests/wikify/test_campaign_driver.py` **NEW** — fixture corpus + 2-iteration campaign + assert the corpus load happens exactly once.

### Verification

- `uv run python -m wikify.cli campaign --strategy M --iterations 3 --budget 50000 --corpus data/corpora/mvp20_v6 --bundle data/test_runs/campaign_next` runs three iterations in one Python process.
...
- `tests/wikify/test_campaign_driver.py` passes.
...
- `src/wikify/contracts/schema.py` — extend `OrchState` with `sampler_snapshot: dict = Field(default_factory=dict)`; no schema change to `OrchAction`
- `src/wikify/distill/policy.py::LlmPolicy`:
...
- `src/wikify/distill/sampler.py` — new `semantic_query_chunks(state, query_vec, k, scope)` helper that does the cosine scan; callable from the handler via Python import
- `.claude/skills/wikify/handlers/orchestrate.md` — rewrite. Document the new `pick_chunks` action, document the two tools (`semantic_query`, `inspect_pages`), show a worked example of query → inspect → pick. Keep the existing sampler actions as a "cheap fallback" section.
- `.claude/skills/wikify/reference/orchestrator.md` — update the action catalog.
- `tests/wikify/test_guided_control_actions.py` — add tests for `pick_chunks` (valid ids, filtered-vs-seen, empty ids) and for the sampler_snapshot construction.
...
- `.claude/skills/wikify/handlers/extract.md` — add an "Image awareness" section: if `images_for_doc` is non-empty AND a caption matches an emitted concept's title/aliases (token overlap or cosine), populate `evidence_figures: [<image_id>]` on that concept. The schema already has this field; nothing uses it today.
- `.claude/skills/wikify/handlers/write.md` — already instructs the writer to mention figures; add a one-line rule: prefer figures whose ID appears in `evidence[].evidence_figures` (which the writer receives via `WriteEvidenceRefV2`).
...
- `src/wikify/ingest/sampler_index.py` **NEW** (shared with Phase 1) — include `caption_chunk_ids` in the index
- `src/wikify/distill/sampler.py` — caption heap, `_global_figures`, tagged residuals
- `src/wikify/distill/policy.py` — `jump_figures` action
- `src/wikify/contracts/schema.py` — `WriteEvidenceRefV2.evidence_figures`
- `src/wikify/distill/write/requests.py` — propagate `evidence_figures` into write request
- `src/wikify/eval/metrics.py` — `image_coverage_residual`, figure-reference counts
- `src/wikify/cli.py::eval_bundle` — emit the new fields
- `.claude/skills/wikify/handlers/extract.md` — image awareness
- `.claude/skills/wikify/handlers/write.md` — prefer evidence-flagged figures
- `.claude/skills/wikify/reference/orchestrator.md` — document `jump_figures`
- `tests/wikify/test_image_sampling.py` **NEW** — captions tagged, `jump_figures` picks from caption heap, image coverage metric returns a float
- `tests/wikify/test_metrics.py` — add `image_coverage_residual` case
...
- `wikify eval` reports non-zero `image_coverage_residual` and `figure_reference_rate`.
...
- `src/wikify/infra/config.py` — `POLL_INTERVAL = 0.05`
- `src/wikify/bindings/file_dispatch.py` — add `extract_many`, keep `extract` as a thin wrapper
- `src/wikify/distill/pipeline.py` — inner extract loop uses `extract_many`
- `src/wikify/contracts/protocols.py` — `Extractor` protocol stays the same (single-request); `extract_many` is a binding-level optimization, not part of the protocol. The pipeline uses `getattr(extractor, 'extract_many', None)` and falls back to the serial path if unavailable.
- `.claude/skills/wikify/runtime/serve-dispatch.md` — note that the runtime skill should handle concurrent requests (poll the whole dispatch dir, spawn handlers in parallel where the hosting Claude Code session supports it)

### Verification (5A)

- New test `tests/wikify/test_extract_many.py` — fake binding + 4-request batch completes in roughly the same wall-clock as 1 request.
...
- `src/wikify/prompts/style_guide.md` + `prompts/fields/*.md` + `prompts/artifact_types/*.md` — compress
- `src/wikify/prompts/registry.py` — new `compose_writer_prompt_layer_hashes(field, artifact) -> dict[str, str]` that returns content hashes alongside the existing string loader
- `src/wikify/contracts/schema.py::WriteRequest` — add `{style_guide_hash, field_guide_hash, artifact_template_hash, corpus_persona_hash}` optional fields; keep the string fields for the fake/heuristic bindings
- `src/wikify/distill/write/requests.py::build_write_request` — populate both the strings and the hashes
- `src/wikify/distill/pipeline.py::run` — write `<bundle>/_meta/prompt_layers/<hash>.md` at startup for each distinct layer
- `src/wikify/paths.py::BundlePaths` — add `prompt_layers_dir` property
- `.claude/skills/wikify/handlers/write.md` — document the hash-based layer fetch; instruct the handler to cache `{hash: text}` between calls; fall back to the inline strings if hashes aren't present
- `.claude/skills/wikify/runtime/serve-dispatch.md` — note that the runtime should keep a session-scoped prompt-layer cache and pass it into write handler invocations
...
- `src/wikify/distill/pipeline.py` — compute `expected_write_reserve`, tighten extract loop condition, pre-check before each write call, record `budget_truncated` entries
- `tests/wikify/test_budget_reservation.py` **NEW** — fake binding + a budget scenario + assert the final spend never exceeds `1.05 × budget_target` and that write reservation is honored
- `tests/wikify/test_pipeline_reallocate.py` — may need an update if the existing assertion depends on the old (leaky) behavior
...
- `.claude/skills/wikify/handlers/write.md` — update the "recommended sections" block to explicitly call out the new hard minimum: "At least two `## H2` sections must precede `## References`. Use sections like `## Definition`, `## Background`, `## Mechanism`, `## Applications`, `## Open Questions`, `## Significance`, etc. Choose what fits the evidence."
- `tests/wikify/test_write_validator.py` — add case: body with H2 only being `## References` is rejected; body with `## Background` + `## Mechanism` + `## References` is accepted.
...
- **Write handler skill**: `.claude/skills/wikify/handlers/write.md` — add a "Writing a person page (`kind=person`)" section. Key rules:
...
- **Tests**:
  - `tests/wikify/test_html_render.py` — add a test that a bundle containing 3 pages (2 with bodies, 1 skeleton) produces only 2 HTML output files.
  - `tests/wikify/test_wiki_index.py` — add a test that skeleton pages are excluded from the index.
...
- `src/wikify/prompts/artifact_types/wiki_concept.md` → rename to `wiki_article.md` (coordinated with 6D) and rewrite
- `src/wikify/prompts/artifact_types/wiki_person.md` — complete rewrite
- `.claude/skills/wikify/handlers/write.md` — update the article + person guidance to reference the new template rules; add the explicit banned-phrases list; reference the Wikipedia MoS URLs as the authoritative source
- `tests/wikify/test_prompts_layered.py` — assertions that the new templates contain the banned-phrases guidance and the required-section language
- `src/wikify/prompts/style_guide.md` — add the banned-phrases list (it's a project-wide rule, not just per-template)
...
- `.claude/skills/wikify/handlers/extract.md` — update the `kind` rule to `"article" | "person"` and update the routing note (`articles/<id>.md` vs `people/<id>.md`)
- `.claude/skills/wikify/handlers/write.md` — same
- `.claude/skills/wikify/reference/orchestrator.md` — update any "concepts" references
...
| File | 6A | 6B | 6C | 6D | 6E |
|---|---|---|---|---|---|
| `src/wikify/contracts/schema.py` | ✓ (validator) | ✓ (WriteRequest.author_context) | | ✓ (kind literal) | |
| `src/wikify/models.py` | | | | ✓ (PageKind) | |
| `src/wikify/paths.py` | | | | ✓ (articles_dir) | |
| `src/wikify/distill/write/author_pages.py` → `author_context.py` | | ✓ (rewrite as context builder) | | | |
| `src/wikify/distill/write/requests.py` | | ✓ (attach author_context) | | | |
| `src/wikify/distill/pipeline.py::_finalize_pages` | | ✓ (drop build_author_pages) | | | |
| `src/wikify/render/html/render.py` | | | ✓ (filter + sane_lists) | ✓ (articles label) | |
| `src/wikify/store/wiki_files.py` | | | | ✓ | |
| `src/wikify/store/wiki_index.py` | | | ✓ (filter) | ✓ (migration) | |
| `src/wikify/prompts/artifact_types/wiki_concept.md` → `wiki_article.md` | | | | ✓ (rename) | ✓ (rewrite) |
| `src/wikify/prompts/artifact_types/wiki_person.md` | | | | | ✓ (rewrite) |
| `src/wikify/prompts/style_guide.md` | | | | | ✓ (banned phrases) |
| `.claude/skills/wikify/handlers/write.md` | ✓ | ✓ (person guidance) | ✓ (list hygiene) | ✓ | ✓ |
| `.claude/skills/wikify/handlers/extract.md` | | | | ✓ | |
| `tests/wikify/test_write_validator.py` | ✓ | | | | |
| `tests/wikify/test_author_context.py` **NEW** | | ✓ | | | |
| `tests/wikify/test_author_pages.py` | | ✓ (delete) | | | |
| `tests/wikify/test_html_render.py` | | | ✓ | ✓ | |
| `tests/wikify/test_wiki_index.py` | | | ✓ | ✓ | |
| `tests/wikify/test_prompts_layered.py` | | | | | ✓ |
...
- Grep `src/wikify/` for `"concept"` in a `kind=` or directory context — zero hits. (The class `ExtractedConcept` stays.)
...
| File | Used by phase |
|---|---|
| `src/wikify/paths.py` | 1, 5B, 6D |
| `src/wikify/models.py` | 6D |
| `src/wikify/ingest/refresh.py` | 1 |
| `src/wikify/ingest/sampler_index.py` **NEW** | 1, 4 |
| `src/wikify/distill/pipeline.py` | 1, 2, 4, 5A, 5B, 5C, 6B |
| `src/wikify/distill/preload.py` **NEW** | 2 |
| `src/wikify/distill/sampler.py` | 1, 3, 4 |
| `src/wikify/distill/policy.py` | 3, 4 |
| `src/wikify/distill/write/requests.py` | 4, 5B, 6B |
| `src/wikify/distill/write/author_pages.py` → `author_context.py` | 6B |
| `src/wikify/contracts/schema.py` | 3, 4, 5B, 6A, 6B, 6D |
| `src/wikify/contracts/protocols.py` | 5A |
| `src/wikify/bindings/file_dispatch.py` | 5A |
| `src/wikify/infra/config.py` | 5A |
| `src/wikify/eval/metrics.py` | 4 |
| `src/wikify/render/html/render.py` | 6C, 6D |
| `src/wikify/store/wiki_files.py` | 6D |
| `src/wikify/store/wiki_index.py` | 6C, 6D |
| `src/wikify/cli.py` | 2, 4 |
| `src/wikify/prompts/registry.py` | 5B |
| `src/wikify/prompts/style_guide.md` + `fields/*.md` + `artifact_types/*.md` | 5B |
| `.claude/skills/wikify/handlers/orchestrate.md` | 3 |
| `.claude/skills/wikify/handlers/extract.md` | 4, 6D |
| `.claude/skills/wikify/handlers/write.md` | 4, 5B, 6A, 6B, 6C, 6D |
| `.claude/skills/wikify/runtime/serve-dispatch.md` | 5A, 5B |
| `.claude/skills/wikify/reference/orchestrator.md` | 3, 4, 6D |
| `.claude/skills/wikify/workflows/run-scripted.md` | 2 |
| `.claude/skills/wikify/workflows/run-campaign.md` | 2 |
...
1. `uv run python -m wikify.cli ingest data/papers/mvp20 --out /tmp/mvp20_check` produces `sampler_index.json`, `pagerank.json`, and the existing artifacts.
2. `uv run python -m wikify.cli campaign --strategy M --iterations 3 --budget 50000 --corpus /tmp/mvp20_check --bundle /tmp/mvp20_bundle` runs all 3 iterations in one Python process.
3. `uv run python -m wikify.cli distill --phase write --bundle /tmp/mvp20_bundle ...` does not load the vector store or corpus graph (verify via stderr instrumentation).
...
5. `uv run python -m wikify.cli eval --bundle /tmp/mvp20_bundle --corpus /tmp/mvp20_check` reports `image_coverage_residual`, `figure_reference_rate`, and `n_figures_referenced_in_bodies` in `_metrics.json`.
...
9. `uv run pytest tests/wikify/ -q` — all tests pass (including the new ones from each phase).
10. `uv run ruff check src/wikify/ tests/wikify/` — clean.
11. `uv run --with pyright pyright src/wikify/` — 0 errors.
12. Wall-time smoke: on mvp20_v6, iteration 1 setup time drops from ~1000 ms to <100 ms.
13. Grep `src/wikify/` for `anthropic` and `openai` — zero new imports from Phase 5 (vendor-neutrality check).
...
15. **Zero `"concept"` in `kind=` or directory contexts** (`grep -rn '"concept"' src/wikify/` in `kind=` or directory scope — zero hits; `ExtractedConcept` class name stays).
16. **Zero deterministic people-page generation**: `grep -rn 'build_author_pages' src/wikify/` — zero hits in production code.
