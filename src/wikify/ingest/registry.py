...
Prefer `wikify.ingest.service` for ingestion and
`wikify.ingest.corpus_refresh` for post-ingest refresh work.
"""
...
def ingest_path(path: Path, parallel: bool = False, max_workers: int = 0) -> int:
    ...
    from wikify.ingest.service import ingest_path as _public_ingest_path
...
def run_batch_steps(new_paper_ids: set[str] | None = None) -> None:
    ...
    from wikify.vault.linker import (
        compute_all_links,
        write_topic_notes,
    )
    from wikify.vault.writer import ensure_vault_dirs, write_paper_note
...
from wikify.ingest import corpus_refresh as _refresh  # noqa: E402
from wikify.ingest import service as _service  # noqa: E402
