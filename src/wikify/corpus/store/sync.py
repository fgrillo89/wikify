...
def write_corpus(
    ...
    *,
    ...
    try:
        ...
        store.fts_rebuild()
        from .metrics import VIEW_CORPUS_CITATION, refresh_cheap_metrics
        from .metrics_global import refresh_h_index, refresh_view

        refresh_cheap_metrics(store.con)
        refresh_view(store.con, VIEW_CORPUS_CITATION)
